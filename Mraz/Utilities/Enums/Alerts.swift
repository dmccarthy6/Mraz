...
enum Alerts {
    ...
    //MARK: - Properties
    static private var userDefaultsStorage: Storage {
        get { return Storage() }
    }
    static private var suppressCloudKitEnabledError: Bool {
        get { return userDefaultsStorage.suppressError ?? false }
    }
    
    //MARK: - Generic Alerts
    static func showAlert(_ viewController: UIViewController, title: AlertTitle, message: AlertMessage) {
    ...
    static func cloudKitErrorAlert(_ error: CloudKitStatusError) {
        let alertController = UIAlertController(title: "iCloud Error", message: error.localizedDescription, preferredStyle: .alert)
        let okAction = UIAlertAction(title: "OK", style: .default, handler: nil)
        alertController.addAction(okAction)
        if let appDel = UIApplication.shared.delegate, let window = appDel.window!, let rootVC = window.rootViewController {
            rootVC.present(alertController, animated: true, completion: nil)
...
enum CloudKitPromptButtonType: String {
    ...
    func performAction() {
        switch self {
        case .OK: break
        case .DontShowAgain: UserDefaults.standard.set(true, forKey: Key.suppressCloudError.rawValue)
        }
    }
